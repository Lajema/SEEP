...
X[,cleaned:=gsub("SEEP_INDIVUDUAL ONE_|SEEP_INDIVUDUAL TWO_|SEEP_INDIVUDUAL THREE_|SEEP_INDIVUDUAL FOUR_","",V1)]
#view(head(X))
names(df)<-X$cleaned
setDT(df)
...
q3$position_in_group <- factor(q3$position_in_group,levels = c("Chairperson","Record Keeper","Box-keeper","Money Counter","Key holder","General member","Respondents","Percentage"))
q3[q3$Country=="-"]<-NA
...
```{r}
today <- "2019/06/28"
dt <- df[,grep("Country|Q_4$",names(df)),with=F]
q4<-staker()
q4[q4$Country=="-"]<-NA
q4$today =today
...
q4$Years_in_Group <- factor(q4$Years_in_Group,levels = c("Less than 1 year","1-5 years","5-10 years","10-15years","Above 16 years","Respondents","Percentage"))
...
```{r}
dt <- df[,grep("Country|Q_5$",names(df)),with=F]
q5<- staker()
q5[q5$Country=="-"]<-NA
...
per <- K%>%  adorn_percentages('col')
per[c(1:length(per$Mali)),2:3] <-per[c(1:length(per$Mali)),2:3] %>% signif(digits=3)
rbind(per,c("Percentage",c(1,1,1,1,1,1)))  %>% kable()
```

# 6. Why are you member of several groups? (Multiple Selection)

```{r}
dt <- df[,grep("Country|A_6",names(df)),with=F]
clean <- function(df){
  df[df==-1]<-NA
    return(df)
...
rbind(K,c("Respondents",607,611))%>%kable()
...
```{r}
dt <- df[,grep("Country|Q_7",names(df)),with=F]
q7<-staker()
q7[q7$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_8$",names(df)),with=F]
q8<-staker()
q8[q8$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_8_S$",names(df)),with=F]
x =melt(dt,id.vars ="Country") %>% drop_na()
x[,value:=tolower(value)]
x[,value:=gsub("flaud","fraud",value)]
x %>% group_by(value,Country) %>% count() %>% dcast(value~Country) %>% select(Mali,Uganda) %>%  kable()
```
...
```{r}
dt <- df[,grep("Country|Q_9$",names(df)),with=F]
q9<-staker()
q9[q9$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_10$",names(df)),with=F]
q10<-staker()
q10[q10$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_11$",names(df)),with=F]
q11 <-staker()
q11[q11$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_12$",names(df)),with=F]
q11<-staker()
q11[q11$Country=="-"]<-NA
...
```{r}
dt <- df[,grep("Country|Q_13$",names(df)),with=F]
q13<-staker()
q13[q13$Country=="-"]<-NA
...
```{r}
dt<-df[,grep("Country|Q_14$",names(df)),with=F]
q14<-staker()
q14[q14$Country=="-"]<-NA
...
# 15.What advantages do you think VSLAs have as compared to bank account?(Multiple Selection)


```{r}
dt<-df[,grep("Country|A_15",names(df)),with=F]
G1<-dt[,c(1:12)]
G2<-dt[,c(1,13:23)]
G3<-dt[,c(1,24:34)]
G4<-dt[,c(1,35:45)]
...
# 16.What advantages do you think bank account has as compared to VSLAs? (Multiple Selection)

```{r}
dt<-df[,grep("Country|A_16",names(df)),with=F]
G1<-dt[,c(1:12)]
G2<-dt[,c(1,13:23)]
G3<-dt[,c(1,24:34)]
G4<-dt[,c(1,35:45)]
...
```{r}
dt<-df[,grep("Country|Q_17",names(df)),with=F]
q17<-staker()
q17[q17$Country=="-"]<-NA
...
```{r}
dt<-df[,grep("Country|Q_18",names(df)),with=F]
q18<-staker()
q18[q18$Country=="-"]<-NA
...
```{r}
dt<-df[,grep("Country|Q_19",names(df)),with=F]
q19<-staker()
q19[q19$Country=="-"]<-NA
...
# 20.What advantages do you think VSLAs have as compared to mobile money? (Multiple Selection)

```{r}
dt<-df[,grep("Country|A_20",names(df)),with=F]
G1<-dt[,c(1:13)]
G2<-dt[,c(1,14:25)]
G3<-dt[,c(1,26:37)]
G4<-dt[,c(1,38:49)]
...
# 21.What advantages do you think mobile money has as compared to VSLAs? (Multiple Selection)

```{r}
dt<-df[,grep("Country|A_21",names(df)),with=F]
G1<-dt[,c(1:11)]
G2<-dt[,c(1,12:21)]
G3<-dt[,c(1,22:31)]
G4<-dt[,c(1,32:41)]
...
```{r}
dt<-df[,grep("Country|Q_22",names(df)),with=F]
q22<-staker()
q22[q22$Country=="-"]<-NA
...
# 23.What was the reason(s) for the VSLA(s) to stop altogether? (Multiple Selection)

```{r}
dt<-df[,grep("Country|A_23",names(df)),with=F]
G1<-dt[,c(1:11)]
G2<-dt[,c(1,12:21)]
G3<-dt[,c(1,22:31)]
G4<-dt[,c(1,32:41)]
...
```{r}
dt<-df[,grep("Country|Q_24",names(df)),with=F]
q24<-staker()
q24[q24$Country=="-"]<-NA
...
```{r}
dt<-df[,grep("Country|Q_25",names(df)),with=F]
q25<-staker()
q25[q25$Country=="-"]<-NA
